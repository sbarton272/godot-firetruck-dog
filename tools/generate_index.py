...
# dependencies = []
# ///
"""Regenerate docs/index.html from docs/versions.json."""
import html
import json
import pathlib
import subprocess
...
def main(argv: list[str]) -> int:
    root = repo_root()
    versions_path = pathlib.Path(argv[0]) if len(argv) > 0 else root / "docs" / "versions.json"
    output_path = pathlib.Path(argv[1]) if len(argv) > 1 else root / "docs" / "index.html"
